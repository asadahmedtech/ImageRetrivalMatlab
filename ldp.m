function histFeature = ldp(img,subregionX,subregionY,bins,test)
    load trees
    img = im2double(img);
%     img = rgb2gray(img);
%     img = imresize(img,[88 88]);
    ...
    for i = 0:subregionY-1
        for j = 0:subregionX-1
            ...
            if test == 1
                histFeature(:,k)= histCounts((LBPFeatureExtract(queryImage)),bins);
            elseif test == 2
